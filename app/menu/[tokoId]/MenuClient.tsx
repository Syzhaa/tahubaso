...
export default function MenuClient({ menus, tokoId }: { menus: Menu[], tokoId: string }) {
  ...
  const renderTracking = () => (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-60 flex items-center justify-center p-4 z-50">
      <div className="text-center p-6 sm:p-8 bg-white rounded-2xl max-w-md mx-auto shadow-2xl w-full">
        <div className="mb-6">
          ...
          <div className="flex justify-center mb-4">
            <div
              className={`w-4 h-4 rounded-full ${
                currentOrder?.status === 'baru' ? 'bg-yellow-500 animate-pulse' : 'bg-green-500'
              }`}
            ></div>
            ...
            <div
              className={`w-4 h-4 rounded-full ${
                currentOrder?.status === 'selesai' ? 'bg-green-500' : 'bg-gray-300'
              }`}
            ></div>
          </div>
        </div>
  ...
  const renderSuccess = () => (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-60 flex items-center justify-center p-4 z-50">
      <div className="text-center p-6 sm:p-8 bg-white rounded-2xl max-w-md mx-auto shadow-2xl transform transition-all scale-100 opacity-100">
        ...
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Pesanan Berhasil!</h2>
        <p className="mt-3 text-gray-600 text-sm sm:text-base mb-6">
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="w-full bg-blue-600 text-white py-3 px-6 rounded-lg font-semibold hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 transition-transform transform hover:scale-105"
          >
            📍 Lacak Pesanan
          </button>
          <button
  ...
  const renderQRIS = () => {
    const qrisImageUrl =
      'https://csczaropdidefplsylwt.supabase.co/storage/v1/object/public/menu-images//qr.jpg';
    return (
      <div className="fixed inset-0 bg-gray-900 bg-opacity-60 flex items-center justify-center p-4 z-50">
        <div className="relative text-center p-6 sm:p-8 bg-white rounded-2xl max-w-sm mx-auto shadow-2xl w-full">
          <button
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Bayar dengan QRIS</h2>
          ...
          <div className="p-2 bg-gray-100 rounded-lg inline-block">
            <Image
              ...
              onError={(e) => {
                e.currentTarget.src = 'https://placehold.co/250x250/e2e8f0/e2e8f0?text=QR';
              }}
            />
          </div>
          <div className="mt-4 p-3 bg-blue-50 border-l-4 border-blue-500 text-blue-800">
            <span className="text-sm">Total Pembayaran</span>
            <strong className="block text-lg sm:text-2xl font-bold">
          ...
          <button
            ...
            className="mt-6 w-full bg-green-500 text-white py-3 rounded-lg font-semibold text-base sm:text-lg hover:bg-green-600 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all"
          >
  ...
  const renderPayment = () => (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-60 flex items-center justify-center p-4 z-50">
      <div className="relative bg-white rounded-2xl max-w-md mx-auto shadow-2xl w-full p-6 sm:p-8">
        <button
          onClick={() => setStep('menu')}
        ...
        <div className="space-y-4">
          <button
            ...
            disabled={isSubmitting}
            className="w-full flex items-center justify-center gap-3 bg-blue-500 text-white py-4 rounded-xl text-base sm:text-lg font-semibold hover:bg-blue-600 disabled:bg-gray-400 transition-all transform hover:scale-105"
          >
            <span className="text-xl sm:text-2xl">💰</span> Bayar di Kasir
          </button>
          <button
            onClick={() => setStep('qris')}
            disabled={isSubmitting}
            className="w-full flex items-center justify-center gap-3 bg-purple-500 text-white py-4 rounded-xl text-base sm:text-lg font-semibold hover:bg-purple-600 disabled:bg-gray-400 transition-all transform hover:scale-105"
          >
            <span className="text-xl sm:text-2xl">📱</span> Bayar dengan QRIS
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <div className="w-full lg:w-3/5 xl:w-2/3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              {menus.map((menu) => (
                <div
                  key={menu.id}
                  className="bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden group"
                >
                  {menu.imageUrl && (
                    <div className="relative w-full h-48 sm:h-56 overflow-hidden">
                      <Image
                        ...
                        onError={(e) => {
                          e.currentTarget.src = `https://placehold.co/400x300/e0e0e0/777?text=${menu.name.replace(
                            /\s/g,
                            '+'
                          )}`;
                        }}
                  ...
                  <div className="p-4 sm:p-5 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto flex justify-between items-center pt-3">
                      ...
                      <button
                        ...
                        className="bg-blue-600 text-white px-4 sm:px-5 py-2 rounded-lg font-semibold hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 transition-transform transform hover:scale-105 text-sm sm:text-base"
                      >
                        + Tambah
                      </button>
                    </div>
        ...
        {cart.length > 0 && (
          <div className="lg:hidden fixed bottom-4 right-4 z-40">
            <button
              ...
              className="bg-blue-600 text-white rounded-full shadow-lg p-4 flex items-center justify-center relative"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        {showCart && (
          ...
          >
            <div
              className="fixed bottom-0 left-0 right-0 bg-white rounded-t-2xl p-4 sm:p-6 shadow-2xl max-h-[80vh] overflow-y-auto"
    ...
    <>
      ...
      {cart.length === 0 ? (
        <div className="text-center py-12 sm:py-16 px-4 sm:px-6">
          ...
          <p className="text-xs sm:text-sm text-gray-400">Silakan tambahkan menu.</p>
        </div>
      ) : (
        <>
              ...
              {cart.map((item) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 bg-gray-100 rounded-full">
                    ...
                    <button
                      onClick={() =>
                        addToCart({ id: item.menuId, name: item.name, price: item.price } as Menu)
                      }
                  ...
                  <p className="w-20 sm:w-24 text-right font-semibold text-gray-700 text-sm sm:text-base">
          ...
          <div className="p-4 sm:p-5 border-t border-gray-200">
            ...
            <button
              ...
              className="w-full bg-green-500 text-white py-3.5 rounded-lg hover:bg-green-600 transition-all font-semibold text-base sm:text-lg flex items-center justify-center gap-2 transform hover:scale-105"
            >
